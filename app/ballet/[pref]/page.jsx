...
import { BALLET_PREFS, ALL_PREFS_LIST } from "../balletData";

export function generateStaticParams() {
  return Object.keys(BALLET_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = BALLET_PREFS[params.pref];
  if (!data) return { title: "バレエ教室 | コドモならいごと" };
  return {
    title: `${data.name}の子供バレエ教室おすすめ｜料金・口コミ比較【コドモならいごと】`,
    description: `${data.name}の子ども向けバレエ教室を比較。Kバレエスクール・チャコットなど人気スクールの月謝・コース・口コミを掲載。無料体験情報も。`,
    keywords: [`${data.name} バレエ教室`, `${data.name} 子供 バレエ`, `キッズバレエ ${data.name}`, "幼児 バレエ 習い事"],
    alternates: { canonical: `https://www.kodomo-navi.com/ballet/${params.pref}` },
  };
}
...
export default function BalletPrefPage({ params }) {
  const { pref } = params;
  const data = BALLET_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}のバレエ教室`, item: `https://www.kodomo-navi.com/ballet/${pref}` },
    ],
  };
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
              ...
              {data.areas.map((area) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span>📍 {area.name}</span>
                    <span style={{ color: "#E91E63", fontSize: 18 }}>›</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>バレエ教室を見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/ballet/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/ballet" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #880E4F, #C2185B)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <Link href="/ballet" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "linear-gradient(135deg, #880E4F, #C2185B)", borderRadius: 20, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none", boxShadow: "0 4px 16px rgba(233,30,99,.3)" }}>
            ← バレエトップへ戻る
          </Link>
        </div>
      </div>
