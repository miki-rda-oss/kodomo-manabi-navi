...
import { BASKETBALL_PREFS, ALL_PREFS_LIST } from "../basketballData";

export function generateStaticParams() {
  return Object.keys(BASKETBALL_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = BASKETBALL_PREFS[params.pref];
  if (!data) return { title: "バスケットボールスクール・教室 | コドモならいごと" };
  return {
    title: `${data.name}の子供バスケットボールスクールおすすめ｜料金・口コミ比較【コドモならいごと】`,
    description: `${data.name}の子ども向けバスケットボールスクール・教室を比較。Bリーグ関連スクール・地域バスケクラブなど人気スクールの月謝・コース・口コミを掲載。無料体験情報も。`,
    keywords: [`${data.name} バスケットボール教室`, `${data.name} 子供 バスケ`, `キッズバスケット ${data.name}`, "ミニバスケット 小学生"],
    alternates: { canonical: `https://www.kodomo-navi.com/basketball/${params.pref}` },
  };
}
...
export default function BasketballPrefPage({ params }) {
  const { pref } = params;
  const data = BASKETBALL_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}のバスケットボールスクール・教室`, item: `https://www.kodomo-navi.com/basketball/${pref}` },
    ],
  };
  ...
  return (
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <section style={{ background: "linear-gradient(135deg, #BF360C, #E65100)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.9)", lineHeight: 1.9 }}>
            {data.heroText}<br />
            市区町村を選んで、お近くのバスケットボールスクールをチェックしよう。
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
              {data.areas.map((area) => (
                <Link
                  key={area.slug}
                  href={`/basketball/${pref}/${area.slug}`}
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>バスケットボールスクールを見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #E6510020" }}>
            🗾 他の都道府県のバスケットボールスクール情報
          </h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/basketball/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/basketball" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #BF360C, #E65100)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <Link href="/basketball" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "linear-gradient(135deg, #BF360C, #E65100)", borderRadius: 20, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none", boxShadow: "0 4px 16px rgba(230,81,0,.3)" }}>
            ← バスケットボールトップへ戻る
          </Link>
        </div>
      </div>
