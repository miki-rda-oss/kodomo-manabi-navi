...
export const metadata = {
  title: '岐阜県の子供向けダンス教室・習い事一覧｜コドモならいごと',
...
const areas = [
  { slug: 'gifu-city', name: '岐阜市' },
  { slug: 'kani', name: '可児市' },
  { slug: 'minokamo', name: '美濃加茂市' },
  { slug: 'tajimi', name: '多治見市' },
];

const faqs = [
  { q: '岐阜県内の子供ダンス教室の月謝相場は？', a: '岐阜県の子供ダンス教室は月4,000円〜7,000円程度が相場です。都市部より比較的リーズナブルな傾向があります。' },
  { q: '岐阜市でおすすめの子供ダンス教室は？', a: '岐阜市内はリディアダンスアカデミーをはじめ複数の教室が展開しています。JR岐阜駅周辺と岐阜市北部に教室が集まっています。' },
  { q: '可児市・美濃加茂市の習い事情報はありますか？', a: 'はい。岐阜県中濃地区の可児市・美濃加茂市の習い事情報を掲載しています。名古屋へのアクセスも良い地域です。' },
  { q: '岐阜県でダンス教室を探すコツは？', a: '岐阜県はJR・名鉄沿線沿いに教室が多い傾向があります。駅からの距離・駐車場の有無を確認しながら選ぶのがポイントです。' },
];

const blogs = [
  { href: '/blog/gifu-kids-dance-school-ranking', text: '岐阜で子供のダンス教室おすすめランキング【2026年版】' },
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <span style={{ fontSize: 13, color: "#888", fontWeight: 600 }}>岐阜県</span>
        </div>
      </header>
      ...
      <section style={{ background: "linear-gradient(135deg, #1B2A4A, #2a4070)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "#fff", marginBottom: 12 }}>岐阜県の子供向け<br />ダンス教室・習い事</h1>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.85)", lineHeight: 1.9 }}>
            清流と山に囲まれた岐阜でお子さまの習い事を探そう<br />
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 18, paddingBottom: 12, borderBottom: "2px solid #FF8A0020" }}>📍 岐阜県のエリアを選ぶ</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {areas.map((area) => (
              <Link key={area.slug} href={`/gifu/${area.slug}`} style={{ display: "block", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 12, padding: "16px 18px", textDecoration: "none", color: "#1B2A4A", fontWeight: 700, fontSize: 15, boxShadow: "0 2px 8px rgba(0,0,0,.04)" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 18, paddingBottom: 12, borderBottom: "2px solid #FF8A0020" }}>❓ よくある質問（岐阜の習い事）</h2>
        ...
        <section style={{ maxWidth: 900, margin: "0 auto 40px", padding: "0 0px" }}>
          <div style={{ background: "linear-gradient(135deg, #fff7ee, #fff0f5)", borderRadius: 16, padding: "24px 20px", border: "1.5px solid #FFD9A0" }}>
            <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 12 }}>
              🥇 岐阜の習い事トレンド2026：ダンスが1位
            </h2>
            <p style={{ fontSize: 14, lineHeight: 1.9, color: "#444", marginBottom: 16 }}>
              岐阜でもキッズダンスが人気習い事に。K-POPブームと中学校ダンス必修化の影響で、岐阜でも子供のダンス習い事への需要が急増しています。
        ...
        <section style={{ background: '#fff', borderRadius: 16, padding: '24px 20px', border: '1.5px solid #e8edf4', marginTop: 32 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[
              ...
              ['/ballet/gifu', '🩰', 'バレエ'],
              ['/gymnastics/gifu', '🤸', '体操'],
              ['/basketball/gifu', '🏀', 'バスケット'],
              ['/preschool/gifu', '🧒', '幼児教室'],
              ['/soroban/gifu', '🧮', 'そろばん'],
              ['/programming/gifu', '💻', 'プログラミング'],
              ['/rhythmics/gifu', '🎵', 'リトミック'],
              ['/music/gifu', '🎸', '音楽'],
              ['/karate/gifu', '🥋', '空手'],
