...
export const metadata = {
  title: '石川県の子供向けダンス教室・習い事一覧｜コドモならいごと',
...
const areas = [
  { slug: 'kanazawa', name: '金沢市' },
  { slug: 'nonoichi', name: '野々市市' },
  { slug: 'uchinada', name: '内灘町' },
];

const faqs = [
  { q: '石川県内の子供ダンス教室の月謝相場は？', a: '石川県の子供ダンス教室は月4,500円〜7,500円程度が相場です。金沢市内は教室数が多く選択肢が豊富です。' },
  { q: '金沢市でおすすめの子供ダンス教室は？', a: '金沢市はリディアダンスアカデミーをはじめ複数の教室が展開しています。JR金沢駅周辺と香林坊・片町エリアに教室が集まっています。' },
  { q: '野々市市・内灘町の習い事情報はありますか？', a: 'はい。野々市市・内灘町の習い事情報を掲載しています。金沢市に隣接した住宅地として子育て世帯が多い地域です。' },
  { q: '石川県でダンス以外の習い事情報はありますか？', a: 'はい。スイミング・英語・ピアノ・空手など幅広い習い事情報を石川各エリアで掲載しています。' },
];
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <span style={{ fontSize: 13, color: "#888", fontWeight: 600 }}>石川県</span>
        </div>
      </header>
      ...
      <section style={{ background: "linear-gradient(135deg, #1B2A4A, #2a4070)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "#fff", marginBottom: 12 }}>石川県の子供向け<br />ダンス教室・習い事</h1>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.85)", lineHeight: 1.9 }}>
            伝統文化の薫る金沢・石川でお子さまの習い事を探そう<br />
            市・町を選んで、お近くの教室情報をチェックしよう。
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 18, paddingBottom: 12, borderBottom: "2px solid #FF8A0020" }}>📍 石川県のエリアを選ぶ</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {areas.map((area) => (
              <Link key={area.slug} href={`/ishikawa/${area.slug}`} style={{ display: "block", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 12, padding: "16px 18px", textDecoration: "none", color: "#1B2A4A", fontWeight: 700, fontSize: 15, boxShadow: "0 2px 8px rgba(0,0,0,.04)" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 18, paddingBottom: 12, borderBottom: "2px solid #FF8A0020" }}>❓ よくある質問（石川の習い事）</h2>
