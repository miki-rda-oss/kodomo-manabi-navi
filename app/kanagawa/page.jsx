...
export const metadata = {
  title: '神奈川県の子供向けダンス教室・習い事一覧｜コドモならいごと',
...
const areas = [
  { slug: 'aoba', name: '横浜市青葉区' },
  { slug: 'atsugi', name: '厚木市' },
  { slug: 'ebina', name: '海老名市' },
  { slug: 'kohoku', name: '横浜市港北区' },
  { slug: 'midori', name: '横浜市緑区' },
  { slug: 'nakahara', name: '川崎市中原区' },
  { slug: 'nakahara-shinjou', name: '中原区新城' },
  { slug: 'sagamihara', name: '相模原市' },
  { slug: 'takatsu', name: '川崎市高津区' },
  { slug: 'tsuzuki', name: '横浜市都筑区' },
  { slug: 'yamato', name: '大和市' },
  { slug: 'zushi', name: '逗子市' },
];

const faqs = [
  { q: '神奈川県内の子供ダンス教室の月謝相場は？', a: '神奈川県の子供ダンス教室は月5,500円〜9,000円程度が相場です。横浜市内は教室数が特に多く、料金・スタイルともに選択肢が豊富です。' },
  { q: '横浜市内でおすすめの子供ダンス教室は？', a: '横浜市内は青葉区・港北区・緑区・都筑区に多数のダンス教室があります。リディアダンスアカデミーも横浜エリアに複数校展開しています。' },
  { q: '川崎市内の子供ダンス教室を教えてください', a: '川崎市は中原区・高津区に人気の教室が集まっています。当サイトでは各区のページから詳細情報をご確認いただけます。' },
  { q: '神奈川でダンス以外の習い事情報もありますか？', a: 'はい。ダンスのほかスイミング・英語・ピアノ・サッカーなど11ジャンルの習い事情報を神奈川各エリアで掲載しています。' },
];
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <span style={{ fontSize: 13, color: "#888", fontWeight: 600 }}>神奈川県</span>
        </div>
      </header>
      ...
      <nav aria-label="パンくず" style={{ maxWidth: 1100, margin: "0 auto", padding: "10px 24px" }}>
        <div style={{ fontSize: 12, color: "#888" }}>
          ...
          {" › "}
          <span style={{ color: "#666" }}>神奈川県</span>
        </div>
      </nav>
      ...
      <section style={{ background: "linear-gradient(135deg, #1B2A4A, #2a4070)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "#fff", marginBottom: 12 }}>神奈川県の子供向け<br />ダンス教室・習い事</h1>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.85)", lineHeight: 1.9 }}>
            横浜・川崎を擁する神奈川でお子さまの習い事を探そう<br />
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {areas.map((area) => (
              <Link key={area.slug} href={`/kanagawa/${area.slug}`} style={{ display: "block", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 12, padding: "16px 18px", textDecoration: "none", color: "#1B2A4A", fontWeight: 700, fontSize: 15, boxShadow: "0 2px 8px rgba(0,0,0,.04)" }}>
        ...
        <section style={{ maxWidth: 900, margin: "0 auto 40px", padding: "0 0px" }}>
          <div style={{ background: "linear-gradient(135deg, #fff7ee, #fff0f5)", borderRadius: 16, padding: "24px 20px", border: "1.5px solid #FFD9A0" }}>
            <h2 style={{ fontSize: 18, fontWeight: 900, color: "#1B2A4A", marginBottom: 12 }}>
              🥇 神奈川の習い事トレンド2026：ダンスが1位
            </h2>
            <p style={{ fontSize: 14, lineHeight: 1.9, color: "#444", marginBottom: 16 }}>
              神奈川・横浜・川崎でダンス人気急上昇。K-POPブームと中学校ダンス必修化の影響で、神奈川でも子供のダンス習い事への需要が急増しています。
              2026年の習い事人気ランキングでダンスが1位を獲得し、スイミングや英語を上回る勢いです。
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {[
                { slug: "kanagawa-kids-dance-2026-guide", title: "神奈川キッズダンス2026年ガイド" },
                { slug: "kanagawa-kids-dance-school", title: "神奈川おすすめダンス教室" },
              ].map(({ slug, title }) => (
        ...
        <section style={{ background: '#fff', borderRadius: 16, padding: '24px 20px', border: '1.5px solid #e8edf4', marginTop: 32 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[
              ['/soccer/kanagawa', '⚽', 'サッカー'],
              ['/piano/kanagawa', '🎹', 'ピアノ'],
              ['/swimming/kanagawa', '🏊', 'スイミング'],
              ['/english/kanagawa', '🌍', '英語'],
              ['/ballet/kanagawa', '🩰', 'バレエ'],
              ['/gymnastics/kanagawa', '🤸', '体操'],
              ['/basketball/kanagawa', '🏀', 'バスケット'],
              ['/preschool/kanagawa', '🧒', '幼児教室'],
              ['/soroban/kanagawa', '🧮', 'そろばん'],
              ['/programming/kanagawa', '💻', 'プログラミング'],
              ['/rhythmics/kanagawa', '🎵', 'リトミック'],
              ['/music/kanagawa', '🎸', '音楽'],
              ['/karate/kanagawa', '🥋', '空手'],
