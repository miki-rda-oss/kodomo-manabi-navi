...
import { PRESCHOOL_PREFS, ALL_PREFS_LIST } from "../preschoolData";

export function generateStaticParams() {
  return Object.keys(PRESCHOOL_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = PRESCHOOL_PREFS[params.pref];
  if (!data) return { title: "幼児教室 | コドモならいごと" };
  return {
    title: `${data.name}の幼児教室・知育教室おすすめ｜料金比較`,
    description: `${data.name}の子ども向け幼児教室・知育教室を比較。七田式・ベビーパーク・公文・ヤマハ・学研など人気教室の月謝・コース・口コミを掲載。無料体験情報も。`,
    keywords: [`${data.name} 幼児教室`, `${data.name} 子供 知育`, `幼児教育 ${data.name}`, "幼児教室 比較"],
    alternates: { canonical: `https://www.kodomo-navi.com/preschool/${params.pref}` },
  };
}
...
export default function PreschoolPrefPage({ params }) {
  const { pref } = params;
  const data = PRESCHOOL_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}の幼児教室`, item: `https://www.kodomo-navi.com/preschool/${pref}` },
    ],
  };
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <Link href="/preschool" style={{ fontSize: 13, color: "#FF8F00", textDecoration: "none", fontWeight: 600 }}>幼児教室・知育</Link>
          <span style={{ color: "#ddd" }}>›</span>
              ...
              {data.areas.map((area) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span>📍 {area.name}</span>
                    <span style={{ color: "#FF8F00", fontSize: 18 }}>›</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>幼児教室を見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/preschool/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/preschool" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #E65100, #FF8F00)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
