...
import { PROGRAMMING_PREFS, ALL_PREFS_LIST } from "../programmingData";

export function generateStaticParams() {
  return Object.keys(PROGRAMMING_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = PROGRAMMING_PREFS[params.pref];
  if (!data) return { title: "プログラミング教室 | コドモならいごと" };
  return {
    title: `${data.name}のプログラミング教室おすすめ｜料金・口コミ比較【コドモならいごと】`,
    description: `${data.name}の子ども向けプログラミング・パソコン教室を比較。LITALICOワンダー・Tech Kids School・ヒューマンアカデミーなど人気スクールの月謝・コース・口コミを掲載。無料体験情報も。`,
    keywords: [`${data.name} プログラミング教室`, `${data.name} 子供 パソコン教室`, `キッズ プログラミング ${data.name}`, "ロボット教室 小学生"],
    alternates: { canonical: `https://www.kodomo-navi.com/programming/${params.pref}` },
  };
}
...
export default function ProgrammingPrefPage({ params }) {
  const { pref } = params;
  const data = PROGRAMMING_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}のプログラミング教室`, item: `https://www.kodomo-navi.com/programming/${pref}` },
    ],
  };
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <Link href="/programming" style={{ fontSize: 13, color: "#1565C0", textDecoration: "none", fontWeight: 600 }}>プログラミング・パソコン教室</Link>
          <span style={{ color: "#ddd" }}>›</span>
      ...
      <section style={{ background: "linear-gradient(135deg, #0D47A1, #1565C0)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.9)", lineHeight: 1.9 }}>
            {data.heroText}<br />
            市区町村を選んで、お近くのプログラミング教室をチェックしよう。
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
              {data.areas.map((area) => (
                <Link
                  key={area.slug}
                  href={`/programming/${pref}/${area.slug}`}
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>プログラミング教室を見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/programming/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/programming" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #0D47A1, #1565C0)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <Link href="/programming" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "linear-gradient(135deg, #0D47A1, #1565C0)", borderRadius: 20, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none", boxShadow: "0 4px 16px rgba(21,101,192,.3)" }}>
            ← プログラミング・パソコン教室トップへ戻る
          </Link>
        </div>
      </div>
