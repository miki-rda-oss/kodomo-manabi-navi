...
export const metadata = {
  title: '島根県の子供向けダンス教室・習い事一覧｜コドモならいごと',
  description: '島根県内のキッズダンス教室・子ども向け習い事を比較。松江市 など主要エリアの教室情報を掲載。',
  keywords: ['島根', '子供', 'ダンス教室', '習い事'],
  alternates: { canonical: 'https://kodomo-manabi-navi.vercel.app/shimane' },
};
export default function PrefPage() {
  const breadcrumb = {"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"トップ","item":"https://kodomo-manabi-navi.vercel.app"},{"@type":"ListItem","position":2,"name":"島根県の習い事","item":"https://kodomo-manabi-navi.vercel.app/shimane"}]};
    ...
    <div style={{fontFamily:"'Noto Sans JP',sans-serif",background:"#f4f7fc",minHeight:"100vh"}}>
      ...
      <header style={{background:"#fff",borderBottom:"1px solid #e8edf4",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",gap:10,height:56}}>
          ...
          <span style={{color:"#ddd"}}>›</span><span style={{fontSize:13,color:"#888"}}>島根県</span>
        </div>
      </header>
      <nav style={{maxWidth:1100,margin:"0 auto",padding:"10px 24px",fontSize:12,color:"#888"}}>
        <Link href="/" style={{color:"#FF8A00",textDecoration:"none"}}>トップ</Link>{" › "}島根県
      </nav>
      <section style={{background:"linear-gradient(135deg,#1B2A4A,#2a4070)",padding:"44px 24px 40px"}}>
        <div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
          <h1 style={{fontSize:28,fontWeight:900,color:"#fff",marginBottom:12}}>島根県の子供向け<br/>ダンス教室・習い事</h1>
          <p style={{fontSize:14,color:"rgba(255,255,255,.85)",lineHeight:1.9}}>松江市を中心に島根県全域の習い事情報を掲載。</p>
        </div>
      </section>
      <div style={{maxWidth:900,margin:"0 auto",padding:"32px 24px 60px"}}>
        <div style={{background:"#fff",borderRadius:18,padding:"28px 24px",marginBottom:24,border:"1.5px solid #e8edf4"}}>
          <h2 style={{fontSize:18,fontWeight:900,color:"#1B2A4A",marginBottom:16}}>📍 島根県のエリアを選ぶ</h2>
          <p style={{fontSize:14,color:"#666",lineHeight:1.9}}>島根県内の市区町村別の習い事情報を準備中です。まずはリディアダンスアカデミーの無料体験をお試しください。</p>
        </div>
        ...
        <section style={{marginBottom:40}}>
          <div style={{background:"linear-gradient(135deg,#fff7ee,#fff0f5)",borderRadius:16,padding:"24px 20px",border:"1.5px solid #FFD9A0"}}>
            <h2 style={{fontSize:18,fontWeight:900,color:"#1B2A4A",marginBottom:12}}>🥇 島根県の習い事トレンド2026：ダンスが1位</h2>
            <p style={{fontSize:14,lineHeight:1.9,color:"#444"}}>K-POPブームと中学校ダンス必修化で島根県でも子供のダンス需要が急増。2026年ランキング1位。</p>
          </div>
        </section>
