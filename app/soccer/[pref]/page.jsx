...
import { SOCCER_PREFS, ALL_PREFS_LIST } from "../soccerData";

export function generateStaticParams() {
  return Object.keys(SOCCER_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = SOCCER_PREFS[params.pref];
  if (!data) return { title: "サッカースクール | コドモならいごと" };
  return {
    title: `${data.name}の子供サッカースクール・教室おすすめ｜料金・口コミ比較【コドモならいごと】`,
    description: `${data.name}の子ども向けサッカースクールを比較。Jリーグ系・地域クラブの月謝・コース・口コミを掲載。`,
    keywords: [`${data.name} サッカースクール`, `${data.name} 子供 サッカー`, `キッズサッカー ${data.name}`, "Jリーグ系 スクール"],
    alternates: { canonical: `https://www.kodomo-navi.com/soccer/${params.pref}` },
  };
}
...
export default function SoccerPrefPage({ params }) {
  const { pref } = params;
  const data = SOCCER_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}のサッカースクール・教室`, item: `https://www.kodomo-navi.com/soccer/${pref}` },
    ],
  };
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <Link href="/soccer" style={{ fontSize: 13, color: "#4CAF50", textDecoration: "none", fontWeight: 600 }}>サッカースクール・教室</Link>
          <span style={{ color: "#ddd" }}>›</span>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
              {data.areas.map((area) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>サッカースクールを見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/soccer/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/soccer" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #4CAF50, #2E7D32)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <Link href="/soccer" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "linear-gradient(135deg, #4CAF50, #2E7D32)", borderRadius: 20, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none", boxShadow: "0 4px 16px rgba(76,175,80,.3)" }}>
            ← サッカースクールトップへ戻る
          </Link>
        </div>
      </div>
