...
import { SWIMMING_PREFS, ALL_PREFS_LIST } from "../swimmingData";

export function generateStaticParams() {
  return Object.keys(SWIMMING_PREFS).map((pref) => ({ pref }));
}

export async function generateMetadata({ params }) {
  const data = SWIMMING_PREFS[params.pref];
  if (!data) return { title: "スイミングスクール | コドモならいごと" };
  return {
    title: `${data.name}の子供スイミングスクールおすすめ｜料金・口コミ比較【コドモならいごと】`,
    description: `${data.name}の子ども向けスイミングスクール・水泳教室を比較。コナミスポーツ・ルネサンス・セントラルスポーツなど人気スクールの月謝・コース・口コミを掲載。`,
    keywords: [`${data.name} スイミングスクール`, `${data.name} 子供 水泳教室`, `キッズ水泳 ${data.name}`, "ベビースイミング"],
    alternates: { canonical: `https://www.kodomo-navi.com/swimming/${params.pref}` },
  };
}
...
export default function SwimmingPrefPage({ params }) {
  const { pref } = params;
  const data = SWIMMING_PREFS[pref];
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${data.name}のスイミングスクール`, item: `https://www.kodomo-navi.com/swimming/${pref}` },
    ],
  };
    ...
    <div style={{ fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", background: "#f4f7fc", minHeight: "100vh" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8edf4", boxShadow: "0 2px 8px rgba(0,0,0,.05)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", gap: 10, height: 56 }}>
          ...
          <Link href="/swimming" style={{ fontSize: 13, color: "#0288D1", textDecoration: "none", fontWeight: 600 }}>スイミング・水泳</Link>
          <span style={{ color: "#ddd" }}>›</span>
      ...
      <section style={{ background: "linear-gradient(135deg, #01579B, #0288D1)", padding: "44px 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(255,255,255,.9)", lineHeight: 1.9 }}>
            {data.heroText}<br />
            市区町村を選んで、お近くのスイミングスクールをチェックしよう。
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px" }}>
              ...
              {data.areas.map((area) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span>📍 {area.name}</span>
                    <span style={{ color: "#0288D1", fontSize: 18 }}>›</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4, fontWeight: 400 }}>スイミングスクールを見る</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, padding: "28px 24px", marginBottom: 24, border: "1.5px solid #e8edf4", boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {data.otherPrefs.map((p, i) => (
              <Link key={i} href={`/swimming/${p.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "#fff", border: "1.5px solid #e8edf4", borderRadius: 20, textDecoration: "none", color: "#1B2A4A", fontSize: 13, fontWeight: 600 }}>
                {p.name}
              </Link>
            ))}
            <Link href="/swimming" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "8px 16px", background: "linear-gradient(135deg, #01579B, #0288D1)", border: "none", borderRadius: 20, textDecoration: "none", color: "#fff", fontSize: 13, fontWeight: 700 }}>
              全都道府県を見る →
            </Link>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center" }}>
          <Link href="/swimming" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "linear-gradient(135deg, #01579B, #0288D1)", borderRadius: 20, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none", boxShadow: "0 4px 16px rgba(2,136,209,.3)" }}>
            ← スイミング・水泳トップへ戻る
          </Link>
        </div>
      </div>
