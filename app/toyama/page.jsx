...
export const metadata = {
  title: '富山県の子供向けダンス教室・習い事一覧｜コドモならいごと',
  description: '富山県内のキッズダンス教室・子ども向け習い事を比較。富山市 など主要エリアの教室情報を掲載。',
  keywords: ['富山', '子供', 'ダンス教室', '習い事'],
  alternates: { canonical: 'https://www.kodomo-navi.com/toyama' },
};
export default function PrefPage() {
  const breadcrumb = {"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"トップ","item":"https://www.kodomo-navi.com"},{"@type":"ListItem","position":2,"name":"富山県の習い事","item":"https://www.kodomo-navi.com/toyama"}]};
  const faq = {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"富山県内の子供ダンス教室の月謝相場は？","acceptedAnswer":{"@type":"Answer","text":"富山県の子供ダンス教室は月5,000円〜8,000円程度が相場です。"}},{"@type":"Question","name":"富山県で3歳から通えるダンス教室はありますか？","acceptedAnswer":{"@type":"Answer","text":"はい。富山県内のダンス教室の多くが3歳から入会できるリトルクラスを設けています。"}},{"@type":"Question","name":"富山県の人気ダンス教室はどこですか？","acceptedAnswer":{"@type":"Answer","text":"リディアダンスアカデミーをはじめ、富山県には体験レッスン無料の人気教室が多数あります。"}}]};
  return (
    <div style={{fontFamily:"'Noto Sans JP',sans-serif",background:"#f4f7fc",minHeight:"100vh"}}>
      ...
      <header style={{background:"#fff",borderBottom:"1px solid #e8edf4",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",gap:10,height:56}}>
          ...
          <span style={{color:"#ddd"}}>›</span><span style={{fontSize:13,color:"#888"}}>富山県</span>
        </div>
      </header>
      <nav style={{maxWidth:1100,margin:"0 auto",padding:"10px 24px",fontSize:12,color:"#888"}}>
        <Link href="/" style={{color:"#FF8A00",textDecoration:"none"}}>トップ</Link>{" › "}富山県
      </nav>
      <section style={{background:"linear-gradient(135deg,#1B2A4A,#2a4070)",padding:"44px 24px 40px"}}>
        <div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
          <h1 style={{fontSize:28,fontWeight:900,color:"#fff",marginBottom:12}}>富山県の子供向け<br/>ダンス教室・習い事</h1>
          <p style={{fontSize:14,color:"rgba(255,255,255,.85)",lineHeight:1.9}}>富山市を中心に富山県全域の習い事情報を掲載。</p>
        </div>
      </section>
      <div style={{maxWidth:900,margin:"0 auto",padding:"32px 24px 60px"}}>
        <div style={{background:"#fff",borderRadius:18,padding:"28px 24px",marginBottom:24,border:"1.5px solid #e8edf4"}}>
          <h2 style={{fontSize:18,fontWeight:900,color:"#1B2A4A",marginBottom:16}}>📍 富山県のエリアを選ぶ</h2>
          <p style={{fontSize:14,color:"#666",lineHeight:1.9}}>富山県内の市区町村別の習い事情報を準備中です。まずはリディアダンスアカデミーの無料体験をお試しください。</p>
        </div>
        ...
        <div style={{background:"#fff",borderRadius:18,padding:"28px 24px",marginBottom:24,border:"1.5px solid #e8edf4"}}>
          ...
          <Link href="/blog/chubu-ishikawa-gifu-kids-dance-2026" style={{display:"block",padding:"10px 0",borderBottom:"1px solid #f0f0f0",color:"#1B2A4A",textDecoration:"none",fontSize:14}}>中部・石川・岐阜キッズダンス2026</Link>
          <Link href="/blog/kids-dance-fee-monthly-complete" style={{display:"block",padding:"10px 0",color:"#1B2A4A",textDecoration:"none",fontSize:14}}>子供のダンス教室月謝・費用完全ガイド</Link>
        </div>
        <section style={{marginBottom:40}}>
          <div style={{background:"linear-gradient(135deg,#fff7ee,#fff0f5)",borderRadius:16,padding:"24px 20px",border:"1.5px solid #FFD9A0"}}>
            <h2 style={{fontSize:18,fontWeight:900,color:"#1B2A4A",marginBottom:12}}>🥇 富山県の習い事トレンド2026：ダンスが1位</h2>
            <p style={{fontSize:14,lineHeight:1.9,color:"#444"}}>K-POPブームと中学校ダンス必修化で富山県でも子供のダンス需要が急増。2026年ランキング1位。</p>
          </div>
        </section>
        <div style={{background:"#fff",borderRadius:18,padding:"28px 24px",marginBottom:24,border:"1.5px solid #e8edf4"}}>
          <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
            {[{href:"/osaka",name:"大阪府"},{href:"/tokyo",name:"東京都"},{href:"/aichi",name:"愛知県"},{href:"/kanagawa",name:"神奈川県"}].map((p,i)=><Link key={i} href={p.href} style={{display:"inline-flex",padding:"8px 16px",background:"#fff",border:"1.5px solid #e8edf4",borderRadius:20,textDecoration:"none",color:"#1B2A4A",fontSize:13,fontWeight:600}}>{p.name}</Link>)}
          </div>
        </div>
        {/* ジャンル別に探す */}
        <section style={{background:'#fff',borderRadius:16,padding:'24px 20px',border:'1.5px solid #e8edf4',marginTop:32,marginBottom:24}}>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
            {[
              ['/soccer/toyama','⚽','サッカー'],
              ['/piano/toyama','🎹','ピアノ'],
              ['/swimming/toyama','🏊','スイミング'],
              ['/english/toyama','🌍','英語'],
              ['/ballet/toyama','🩰','バレエ'],
              ['/gymnastics/toyama','🤸','体操'],
              ['/basketball/toyama','🏀','バスケット'],
              ['/preschool/toyama','🧒','幼児教室'],
              ['/soroban/toyama','🧮','そろばん'],
              ['/programming/toyama','💻','プログラミング'],
              ['/rhythmics/toyama','🎵','リトミック'],
              ['/music/toyama','🎸','音楽'],
              ['/karate/toyama','🥋','空手'],
